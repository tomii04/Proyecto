...
if($method === 'POST'){
    ...
    switch($action){
        case 'aprobar_aspirante':
            ...
            $uni = $stmtU->fetch();
            if($uni){
                $stmtUp = $pdo->prepare("UPDATE Socio SET Unidad=? WHERE CIsoc=?");
                $stmtUp->execute([$uni['NomLote'], $asp['CIaspi']]);
...
if($method==='GET' && isset($_GET['accion'])){
    switch($_GET['accion']){
        ...
        case 'socios':
            $stmt = $pdo->query("SELECT CIsoc,PnomS,PapeS,EmailS,TelS,EstadoSoc,FchIngr,Unidad FROM Socio");
            echo json_encode($stmt->fetchAll()); exit;
        default:
